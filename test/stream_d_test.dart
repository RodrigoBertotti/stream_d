...
void main() {
  ...
  test('StreamD from controller: onDone() should be triggered when cancel() is called', () {
    final controller = StreamController();
    final streamD = StreamD(controller: controller);
  ...
  test('StreamD from controller: onDone() should be triggered when controller.close() is called', () {
    final controller = StreamController();
    final streamD = StreamD(controller: controller);
    final subscription = streamD.listenD((event) {});
    subscription.addOnDone(expectAsync0(() => expect('1','1')));
  ...
  test('StreamD from controller: onDone() should be triggered even if more than one subscription is active', () async {
  ...
  test('StreamD from stream: onDone() should be triggered when cancel() is called', () {
    final controller = StreamController();
    final streamD = StreamD.fromStream(controller.stream);
  ...
  test('StreamD from broadcast stream: onDone() should be triggered even if more than one subscription is active', () async {
    final controller = StreamController.broadcast();
    final streamD = StreamD.fromStream(controller.stream);
    bool sub1Called = false;
    final sub1 = streamD.listenD((event) {sub1Called = true;});
